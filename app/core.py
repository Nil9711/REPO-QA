"""
Core Q&A module - callable function for answering questions about indexed repositories.
"""
import sys
...
def _build_query_engine(index_dir: str):
    """Build the query engine for a given index directory."""
...
def _route_question(question: str) -> tuple[str, float]:
    """Route the question to the appropriate mode."""
...
def _format_retrieved_context(response, max_chunks=None) -> str:
    """Format retrieved context from response nodes."""
...
def _extract_sources(response) -> list:
    """Extract source file paths and scores from response."""
    sources = []
...
def _query_with_mode(query_engine, collection, question: str, mode: str) -> tuple[str, list]:
    """Execute query with the specified mode."""
    ...
    if mode == "repo_overview":
        ...
        retrieved_context = _format_retrieved_context(response, max_chunks=max_chunks)
    ...
    retrieved_sources = _extract_sources(response) if response else []
    sources = authoritative_sources + retrieved_sources
    return str(final_response), sources


def ask(index_dir: str, question: str, save_history: bool = True) -> dict:
    """
    Answer a question about an indexed repository.

    Args:
        index_dir: Path to the ChromaDB index directory
        question: The question to answer
        save_history: Whether to save the Q&A to history (default: True)

    Returns:
        dict with keys:
            - answer: str - The generated answer
            - sources: list - List of source files with scores
            - mode: str - The routing mode used (repo_overview, api_endpoints, deep_dive)
            - confidence: float - Router confidence score
    """
    mode, confidence = _route_question(question)
    query_engine, collection = _build_query_engine(index_dir)

    answer, sources = _query_with_mode(query_engine, collection, question, mode)
    ...
    if save_history:
        _save_history(question, answer, sources, index_dir, mode, confidence)

    return result


def _save_history(question: str, answer: str, sources: list, index_dir: str, mode: str, confidence: float):
